...
import { PerformanceTracker, TradeRecord } from "./performance-tracker";
...
export class MarketOptimizationEngine extends EventEmitter {
  ...
  private customRules: OptimizationRule[] = [];
  ...
  constructor(providers: Map<number, JsonRpcProvider>, config?: Partial<OptimizationConfig>) {
    ...
    this.marketAnalyzer = new MarketAnalyzer(providers);
    ...
    this.slippageManager = new DynamicSlippageManager(providers);
  ...
  async initializeOptimization(primaryChainId: number): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to initialize optimization', { error: (error as Error)?.message || error });
      throw error;
    }
  }

  async optimizeParameters(chainId: number): Promise<OptimizedParameters> {
    try {
      ...
      this.logger.error('Parameter optimization failed', { error: (error as Error)?.message || error, chainId });
      throw error;
    }
  }

  private async applyOptimizationLogic(
    ...
    const profitThreshold = await this.adaptiveProfitManager.calculateOptimalThreshold(
      ['0x82aF49447D8a07e3bd95BD0d56f35241523fBab1', '0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8'], // WETH/USDC
      parseEther("1"), // 1 ETH trade size
      parseEther("0.001"), // Estimated gas cost
    ...
    const slippageSettings = await this.slippageManager.calculateOptimalSlippage(
      '0x82aF49447D8a07e3bd95BD0d56f35241523fBab1', // WETH
      '0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8', // USDC
      optimizedParams.maxTradeSize,
      42161 // Arbitrum chain ID
    );
  ...
  private applyMarketProfile(params: OptimizedParameters, conditions: MarketConditions): OptimizedParameters {
    // Select appropriate market profile
    let profile: MarketProfile | undefined;
  ...
  private applySafetyBounds(params: OptimizedParameters): OptimizedParameters {
    const bounds = this.config.safetyBounds;
    
    // Apply safety bounds
    params.minProfitThreshold = this.clampBigInt(
      params.minProfitThreshold, 
      bounds.minProfitThreshold, 
      bounds.maxProfitThreshold
    );
    
    params.slippageTolerance = Math.max(
      bounds.minSlippage, 
      Math.min(params.slippageTolerance, bounds.maxSlippage)
    );
    
    params.maxTradeSize = this.clampBigInt(
      params.maxTradeSize, 
      bounds.minTradeSize, 
      bounds.maxTradeSize
    );
  ...
  private calculateExpectedImprovement(
    previous: OptimizedParameters,
    current: OptimizedParameters,
    ...
    if (profitChange < 0) improvement += Math.abs(profitChange) * 10; // Lower threshold = more opportunities
  ...
  private mapGasUrgency(urgency: number): 'low' | 'medium' | 'high' | 'urgent' {
  ...
  private getParameterChanges(previous: OptimizedParameters, current: OptimizedParameters): string[] {
    const changes: string[] = [];
    
    if (previous.minProfitThreshold !== current.minProfitThreshold) {
      changes.push(`Profit threshold: ${formatEther(previous.minProfitThreshold)} → ${formatEther(current.minProfitThreshold)} ETH`);
    }
    ...
    if (previous.maxTradeSize !== current.maxTradeSize) {
      changes.push(`Max trade size: ${formatEther(previous.maxTradeSize)} → ${formatEther(current.maxTradeSize)} ETH`);
    }
  ...
  private startOptimizationLoop(chainId: number): void {
    ...
    this.optimizationInterval = setInterval(async () => {
      try {
        ...
        this.logger.error('Optimization loop error', { error: (error as Error)?.message || error });
      }
    }, this.config.frequency);
  }

  private initializeConfig(config?: Partial<OptimizationConfig>): OptimizationConfig {
    return {
      ...
      safetyBounds: config?.safetyBounds ?? {
        ...
        maxProfitThreshold: parseEther("1.0"),
        minSlippage: 10, // 0.1%
        maxSlippage: 1000, // 10%
        minTradeSize: parseEther("0.1"),
        maxTradeSize: parseEther("100")
      }
    };
  }
  ...
  private getDefaultParameters(): OptimizedParameters {
    return {
      minProfitThreshold: parseEther("0.01"),
      slippageTolerance: 50, // 0.5%
      maxTradeSize: parseEther("10"),
      gasSettings: {
        maxFeePerGas: parseEther("0.00002"), // 20 gwei
        maxPriorityFeePerGas: parseEther("0.000002"), // 2 gwei
        urgency: 'medium'
      },
  ...
  async forceOptimization(chainId: number): Promise<OptimizationResult> {
