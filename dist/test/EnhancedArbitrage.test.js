...
describe("Enhanced FlashArbBotBalancer Tests", function () {
    ...
    beforeEach(async function () {
        ...
        bot = await FlashArbBotBalancer.deploy(BALANCER_VAULT, SUSHI_ROUTER_NEW, UNI_V2_ROUTER_NEW, UNI_V3_QUOTER);
    ...
    describe("Enhanced Deployment", function () {
        it("Should initialize with correct chain detection", async function () {
            ...
            (0, chai_1.expect)(chainId).to.equal(31337); // Hardhat network
        });
        it("Should have correct token constants", async function () {
            (0, chai_1.expect)(await bot.WETH()).to.equal(WETH_ADDRESS);
            (0, chai_1.expect)(await bot.USDC()).to.equal(USDC_ADDRESS);
            (0, chai_1.expect)(await bot.USDT()).to.equal(USDT_ADDRESS);
            (0, chai_1.expect)(await bot.WBTC()).to.equal(WBTC_ADDRESS);
        });
        it("Should have correct router constants", async function () {
            (0, chai_1.expect)(await bot.UNI_V2_ROUTER_NEW()).to.equal(UNI_V2_ROUTER_NEW);
            (0, chai_1.expect)(await bot.SUSHI_ROUTER_NEW()).to.equal(SUSHI_ROUTER_NEW);
        });
        it("Should detect network correctly", async function () {
            // On Hardhat, both should be false
            (0, chai_1.expect)(await bot.isArbitrum()).to.be.false;
            (0, chai_1.expect)(await bot.isOptimism()).to.be.false;
        });
    });
    describe("Triangular Arbitrage", function () {
        it("Should execute triangular arbitrage with valid path", async function () {
            ...
            await (0, chai_1.expect)(bot.executeTriangularArb(WETH_ADDRESS, amount, triangularPath, expectedProfit)).to.not.be.reverted;
        });
        ...
        it("Should reject invalid triangular paths", async function () {
            ...
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeTriangularArb(WETH_ADDRESS, amount, invalidPath, expectedProfit)).to.be.revertedWith("Triangular path must have 4 tokens (A->B->C->A)");
        });
        it("Should reject non-circular triangular paths", async function () {
            ...
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeTriangularArb(WETH_ADDRESS, amount, nonCircularPath, expectedProfit)).to.be.revertedWith("Path must start and end with same token");
        });
    });
    describe("Enhanced Token Pair Support", function () {
        it("Should support ETH/USDT arbitrage", async function () {
            ...
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, amount, path, true, expectedProfit)).to.not.be.reverted;
        });
        it("Should support WBTC/ETH arbitrage", async function () {
            const amount = hardhat_1.ethers.parseUnits("0.1", 8); // 0.1 WBTC
            const path = [WBTC_ADDRESS, WETH_ADDRESS];
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeArb(WBTC_ADDRESS, amount, path, false, expectedProfit)).to.not.be.reverted;
        });
        it("Should simulate arbitrage for all supported pairs", async function () {
            ...
            for (const pair of pairs) {
                ...
                (0, chai_1.expect)(profitSushiFirst).to.be.a('bigint');
                (0, chai_1.expect)(profitUniFirst).to.be.a('bigint');
            }
        });
    });
    describe("Enhanced Events", function () {
        it("Should have TriangularArbitrageExecuted event", async function () {
            ...
            (0, chai_1.expect)(eventFilter).to.exist;
            (0, chai_1.expect)(eventFilter.fragment?.name).to.equal("TriangularArbitrageExecuted");
        });
        it("Should have CrossChainOpportunityDetected event", async function () {
            ...
            (0, chai_1.expect)(eventFilter).to.exist;
            (0, chai_1.expect)(eventFilter.fragment?.name).to.equal("CrossChainOpportunityDetected");
        });
        it("Should emit correct event parameters for triangular arbitrage", async function () {
            const eventFilter = bot.filters.TriangularArbitrageExecuted();
            const fragment = eventFilter.fragment;
            (0, chai_1.expect)(fragment?.inputs).to.have.lengthOf(5);
            (0, chai_1.expect)(fragment?.inputs[0].name).to.equal("tokenA");
            (0, chai_1.expect)(fragment?.inputs[1].name).to.equal("tokenB");
            (0, chai_1.expect)(fragment?.inputs[2].name).to.equal("tokenC");
            (0, chai_1.expect)(fragment?.inputs[3].name).to.equal("amount");
            (0, chai_1.expect)(fragment?.inputs[4].name).to.equal("profit");
        });
    });
    ...
    describe("Cross-Chain Compatibility", function () {
        it("Should have Optimism router constants", async function () {
            (0, chai_1.expect)(await bot.UNI_V2_ROUTER_OPT()).to.equal("0x4A7b5Da61326A6379179b40d00F57E5bbDC962c2");
            (0, chai_1.expect)(await bot.SUSHI_ROUTER_OPT()).to.equal("0x2ABf469074dc0b54d793850807E6eb5Faf2625b1");
        });
        it("Should handle chain detection correctly", async function () {
            ...
            (0, chai_1.expect)(isArbitrum).to.be.false;
            (0, chai_1.expect)(isOptimism).to.be.false;
        });
    });
    ...
    describe("Error Handling", function () {
        it("Should handle zero amounts gracefully", async function () {
            const path = [WETH_ADDRESS, USDC_ADDRESS];
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, 0, path, true, expectedProfit)).to.be.revertedWith("Amount must be positive");
        });
        it("Should handle zero expected profit", async function () {
            const amount = hardhat_1.ethers.parseEther("1");
            const path = [WETH_ADDRESS, USDC_ADDRESS];
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, amount, path, true, 0)).to.be.revertedWith("Expected profit must be positive");
        });
    });
});
