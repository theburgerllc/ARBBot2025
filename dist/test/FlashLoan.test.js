...
describe("FlashArbBotBalancer Unit Tests", function () {
    ...
    beforeEach(async function () {
        ...
        mockUniRouter = await hardhat_1.ethers.getContractAt("IUniswapV2Router02", UNI_V2_ROUTER);
        mockSushiRouter = await hardhat_1.ethers.getContractAt("IUniswapV2Router02", SUSHI_ROUTER);
    });
    describe("Deployment", function () {
        it("Should set the right owner", async function () {
            (0, chai_1.expect)(await bot.owner()).to.equal(await owner.getAddress());
        });
        it("Should set the correct contract addresses", async function () {
            (0, chai_1.expect)(await bot.vault()).to.equal(BALANCER_VAULT);
            (0, chai_1.expect)(await bot.sushiRouter()).to.equal(SUSHI_ROUTER);
            (0, chai_1.expect)(await bot.uniRouter()).to.equal(UNI_V2_ROUTER);
            (0, chai_1.expect)(await bot.uniV3Quoter()).to.equal(UNI_V3_QUOTER);
        });
        it("Should have correct default parameters", async function () {
            (0, chai_1.expect)(await bot.slippageTolerance()).to.equal(200); // 2%
            (0, chai_1.expect)(await bot.minProfitBps()).to.equal(30); // 0.3%
            (0, chai_1.expect)(await bot.MAX_SLIPPAGE()).to.equal(500); // 5%
        });
    });
    describe("Access Control", function () {
        it("Should allow owner to set authorized caller", async function () {
            ...
            (0, chai_1.expect)(await bot.authorizedCallers(await addr1.getAddress())).to.be.true;
        });
        it("Should not allow non-owner to set authorized caller", async function () {
            await (0, chai_1.expect)(bot.connect(addr1).setAuthorizedCaller(await addr1.getAddress(), true)).to.be.revertedWith("Ownable: caller is not the owner");
        });
        it("Should allow owner to set slippage tolerance", async function () {
            await bot.setSlippageTolerance(300); // 3%
            (0, chai_1.expect)(await bot.slippageTolerance()).to.equal(300);
        });
        it("Should not allow slippage tolerance above maximum", async function () {
            await (0, chai_1.expect)(bot.setSlippageTolerance(600) // 6%
            ).to.be.revertedWith("Slippage too high");
        });
        it("Should allow owner to set minimum profit", async function () {
            await bot.setMinProfitBps(50); // 0.5%
            (0, chai_1.expect)(await bot.minProfitBps()).to.equal(50);
        });
        it("Should not allow zero minimum profit", async function () {
            await (0, chai_1.expect)(bot.setMinProfitBps(0)).to.be.revertedWith("Min profit must be positive");
        });
    });
    describe("Pause Functionality", function () {
        it("Should allow owner to pause and unpause", async function () {
            await bot.pause();
            (0, chai_1.expect)(await bot.paused()).to.be.true;
            await bot.unpause();
            (0, chai_1.expect)(await bot.paused()).to.be.false;
        });
        it("Should prevent execution when paused", async function () {
            ...
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, amount, path, true, expectedProfit)).to.be.revertedWith("Pausable: paused");
        });
    });
    describe("Arbitrage Execution", function () {
        it("Should reject invalid parameters", async function () {
            ...
            const amount = hardhat_1.ethers.parseEther("1");
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, amount, invalidPath, true, expectedProfit)).to.be.revertedWith("Invalid path");
        });
        it("Should reject zero amount", async function () {
            const path = [WETH_ADDRESS, USDC_ADDRESS];
            const amount = 0;
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, amount, path, true, expectedProfit)).to.be.revertedWith("Amount must be positive");
        });
        it("Should reject zero expected profit", async function () {
            const path = [WETH_ADDRESS, USDC_ADDRESS];
            const amount = hardhat_1.ethers.parseEther("1");
            const expectedProfit = 0;
            await (0, chai_1.expect)(bot.executeArb(WETH_ADDRESS, amount, path, true, expectedProfit)).to.be.revertedWith("Expected profit must be positive");
        });
        it("Should only allow authorized callers", async function () {
            const path = [WETH_ADDRESS, USDC_ADDRESS];
            const amount = hardhat_1.ethers.parseEther("1");
            const expectedProfit = hardhat_1.ethers.parseEther("0.01");
            await (0, chai_1.expect)(bot.connect(addr1).executeArb(WETH_ADDRESS, amount, path, true, expectedProfit)).to.be.revertedWith("Not authorized");
        });
    });
    describe("Simulation", function () {
        ...
        it("Should return zero profit for unprofitable trades", async function () {
            const path = [WETH_ADDRESS, USDC_ADDRESS];
            const amount = hardhat_1.ethers.parseEther("1000000"); // Unrealistically large amount
            const profit = await bot.simulateArbitrage(WETH_ADDRESS, amount, path, true);
            (0, chai_1.expect)(profit).to.equal(0n);
        });
    });
    describe("Withdraw Functionality", function () {
        it("Should allow owner to withdraw tokens", async function () {
            // Send some tokens to the contract first
            await mockWETH.mint(await bot.getAddress(), hardhat_1.ethers.parseEther("1"));
            ...
            (0, chai_1.expect)(finalBalance - initialBalance).to.equal(hardhat_1.ethers.parseEther("1"));
        });
        it("Should not allow non-owner to withdraw", async function () {
            await (0, chai_1.expect)(bot.connect(addr1).withdraw(await mockWETH.getAddress())).to.be.revertedWith("Ownable: caller is not the owner");
        });
        it("Should revert when trying to withdraw zero balance", async function () {
            await (0, chai_1.expect)(bot.withdraw(await mockWETH.getAddress())).to.be.revertedWith("No balance to withdraw");
        });
    ...
    describe("Price Feed Integration", function () {
        it("Should allow setting price feeds", async function () {
            const mockPriceFeed = await hardhat_1.ethers.getContractFactory("MockPriceFeed");
            const priceFeed = await mockPriceFeed.deploy();
            await bot.setPriceFeed(WETH_ADDRESS, await priceFeed.getAddress());
            (0, chai_1.expect)(await bot.priceFeeds(WETH_ADDRESS)).to.equal(await priceFeed.getAddress());
        });
        it("Should not allow non-owner to set price feeds", async function () {
            const mockPriceFeed = await hardhat_1.ethers.getContractFactory("MockPriceFeed");
            const priceFeed = await mockPriceFeed.deploy();
            await (0, chai_1.expect)(bot.connect(addr1).setPriceFeed(WETH_ADDRESS, await priceFeed.getAddress())).to.be.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    describe("Events", function () {
        ...
        it("Should emit ProfitWithdrawn event", async function () {
            // Send some tokens to the contract first
            await mockWETH.mint(await bot.getAddress(), hardhat_1.ethers.parseEther("1"));
            await (0, chai_1.expect)(bot.withdraw(await mockWETH.getAddress()))
                .to.emit(bot, "ProfitWithdrawn")
                .withArgs(await mockWETH.getAddress(), hardhat_1.ethers.parseEther("1"));
        });
    });
});
