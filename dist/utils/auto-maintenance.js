...
exports.AutoMaintenanceManager = void 0;
...
class AutoMaintenanceManager {
    ...
    setupLogger() {
        this.logger = winston_1.default.createLogger({
            ...
            transports: [
                ...
                new winston_1.default.transports.File({ filename: 'maintenance.log' })
            ]
        });
    }
    ...
    shouldRunTask(task, now) {
        ...
        if (task.name === 'daily' && hoursSinceLastRun >= 24)
            return true;
        if (task.name === 'weekly' && hoursSinceLastRun >= 168)
            return true; // 7 days
        if (task.name === 'monthly' && hoursSinceLastRun >= 720)
            return true; // 30 days
        return false;
    }
    ...
    async cleanOldLogs() {
        try {
            ...
            const simDir = path_1.default.join(process.cwd(), 'simulation-results');
            try {
                const simFiles = await promises_1.default.readdir(simDir);
                for (const file of simFiles) {
                    if (file.endsWith('.json')) {
                        const filePath = path_1.default.join(simDir, file);
                        const stats = await promises_1.default.stat(filePath);
                        const daysOld = (Date.now() - stats.mtime.getTime()) / (1000 * 60 * 60 * 24);
                        if (daysOld > 7) {
                            await promises_1.default.unlink(filePath);
    ...
    async optimizeCache() {
        try {
            ...
            try {
                const stats = await promises_1.default.stat(cacheDir);
    ...
    async updateTokenPriceCache() {
        try {
            ...
            for (const token of tokens) {
                try {
                    const response = await axios_1.default.get(`https://api.coingecko.com/api/v3/simple/price?ids=${token.coingeckoId}&vs_currencies=usd`, { timeout: 10000 });
    ...
    async sendDailyMaintenanceReport(report) {
        try {
            if (!process.env.DISCORD_WEBHOOK_URL)
                return;
            const payload = {
            ...
            await axios_1.default.post(process.env.DISCORD_WEBHOOK_URL, payload);
        }
        catch (error) {
            this.logger.error('Failed to send daily maintenance report:', error);
        }
    }
    async performDeepSystemOptimization() {
...
exports.AutoMaintenanceManager = AutoMaintenanceManager;
