...
exports.VolatileTokenTracker = void 0;
class VolatileTokenTracker {
    // High-volatility token database with addresses
    static VOLATILE_TOKENS = {
        // Arbitrum (42161)
        42161: [
            {
                symbol: "ETH",
     ...
     */
    static getHighVolatilityPairs(chainId) {
        ...
        if (chainId === 10) {
            priorityPairs.push(["OP", "ETH"], ["SNX", "ETH"], ["PENGU", "ETH"], ["BONK", "ETH"], ["MOG", "ETH"], ["FUN", "ETH"], ["XLM", "ETH"], ["1INCH", "ETH"], ["CVX", "ETH"]);
        }
        ...
        for (let i = 0; i < tokens.length; i++) {
            for (let j = i + 1; j < tokens.length; j++) {
                ...
                const existingPair = pairs.find(p => (p.tokenA.symbol === tokenA.symbol && p.tokenB.symbol === tokenB.symbol) ||
     ...
     */
    static getExpandedTokenUniverse(chainId) {
     ...
     */
    static async fetchLiveVolatilityData(tokens) {
        try {
     ...
     */
    static async updateTokenVolatility(pairs) {
        ...
        return pairs.map(pair => ({
            ...
            expectedVolatility: ((liveVolatility[pair.tokenA.symbol] || pair.tokenA.volatility24h) +
                (liveVolatility[pair.tokenB.symbol] || pair.tokenB.volatility24h)) / 2
        }));
    }
    static createVolatileToken(tokenData, chainId) {
        return {
    ...
    static calculateLiquidityScore(symbolA, symbolB) {
     ...
     */
    static getMostVolatileTokens(chainId, limit = 10) {
