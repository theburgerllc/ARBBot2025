...
exports.DexIntegration = void 0;
const ethers_1 = require("ethers");
const api_1 = require("@curvefi/api");
...
class DexIntegration {
    ...
    async initializeCurve() {
        try {
            ...
            this.curveApi = new api_1.CurveApi({
                network: networkName,
    ...
    async initializeGMX() {
        try {
            ...
            if (this.chainId === 42161 || this.chainId === 43114) { // Arbitrum or Avalanche
                ...
                this.gmxReader = new sdk_1.GMXReader(provider, this.chainId);
    ...
    async getCurveRoutes(tokenIn, tokenOut, amountIn) {
        try {
            if (!this.curveApi) {
            ...
            for (const pool of pools) {
                try {
                    ...
                    const quote = await this.curveApi.getQuote(pool.address, tokenIn, tokenOut, amountIn);
    ...
    async getGMXRoutes(tokenIn, tokenOut, amountIn) {
        try {
            if (!this.gmxReader) {
            ...
            const quote = await this.gmxReader.getSwapQuote({
                ...
                tokenOut: tokenOut,
                amountIn: amountIn,
                receiver: ethers_1.ethers.ZeroAddress // Will be replaced with actual receiver
            });
            if (quote && quote.amountOut) {
                return [{
                        dexName: 'GMX',
    ...
    async getUniswapV3Routes(tokenIn, tokenOut, amountIn) {
        try {
            ...
            const quoter = new ethers_1.ethers.Contract(quoterAddress, quoterABI, provider);
    ...
    getUniswapV3QuoterAddress(chainId) {
    ...
    async getBalancerRoutes(tokenIn, tokenOut, amountIn) {
        try {
      ...
      `;
            const response = await axios_1.default.post(sorUrl, { query });
    ...
    async getAllRoutes(tokenIn, tokenOut, amountIn) {
        ...
        const bestRoute = allRoutes.reduce((best, current) => {
            if (!best)
                return current;
            const bestAmount = ethers_1.ethers.getBigInt(best.amountOut);
            const currentAmount = ethers_1.ethers.getBigInt(current.amountOut);
    ...
    async validateRoute(route) {
        try {
            ...
            return ethers_1.ethers.isAddress(tokenIn) && ethers_1.ethers.isAddress(tokenOut);
        }
        catch (error) {
    ...
    async getOptimalSlippage(route) {
...
exports.DexIntegration = DexIntegration;
