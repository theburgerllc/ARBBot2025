...
class TriangularArbManager {
    ...
    constructor(provider, dexManager, gasOptimizer) {
        this.provider = provider;
        this.dexManager = dexManager;
        this.gasOptimizer = gasOptimizer;
    }
    // MAIN FUNCTION: Scan for triangular arbitrage opportunities
    async scanTriangularOpportunities(chainId, minProfitETH = 0.01, maxPathsToAnalyze = 50) {
        const opportunities = [];
        try {
            ...
            for (const pathCombo of pathsToAnalyze) {
                ...
                if (opportunity && opportunity.profitAfterGas > (0, ethers_1.parseUnits)(minProfitETH.toString(), 18)) {
    ...
    async executeTriangularArb(opportunity, contractAddress) {
        ...
        try {
            ...
            const gasSettings = await this.gasOptimizer.getGasStrategyForOpportunity('triangular', opportunity.profitAfterGas, opportunity.chainId);
            // Create contract instance
            const contract = new ethers_1.Contract(contractAddress, ['function executeTriangularArb(address,uint256,address[],address[],bool[],uint256) external'], this.provider);
            // Execute the transaction
            const tx = await contract.executeTriangularArb(path.path[0], // Starting token
            path.inputAmount, path.path, // Token path
    ...
    generateTriangularPaths(chainId, routers) {
        const paths = [];
        const tokenAddresses = this.TOKEN_ADDRESSES[chainId];
        if (!tokenAddresses)
            return [];
        // Focus on preferred high-volume paths first
        for (const preferredPath of this.PREFERRED_PATHS) {
            const tokens = preferredPath.map(symbol => tokenAddresses[symbol]);
    ...
    async analyzeTriangularPath(pathCombo, chainId, minProfitETH) {
        try {
            const inputAmounts = [
                (0, ethers_1.parseUnits)("1", 18), // 1 ETH equivalent
                (0, ethers_1.parseUnits)("5", 18), // 5 ETH equivalent
                (0, ethers_1.parseUnits)("10", 18) // 10 ETH equivalent
            ];
            const paths = [];
            for (const inputAmount of inputAmounts) {
                const path = await this.simulateTriangularPath(pathCombo.tokens, pathCombo.routers, inputAmount, chainId);
            ...
            const bestPath = paths.reduce((best, current) => current.profitability > best.profitability ? current : best);
            // Calculate gas costs
            ...
            const profitAfterGas = bestPath.expectedOutput > bestPath.inputAmount + gasCost
                ...
                : 0n;
            if (profitAfterGas < (0, ethers_1.parseUnits)(minProfitETH.toString(), 18)) {
                return null;
            }
            // Calculate confidence score
            const confidenceScore = this.calculateConfidenceScore(bestPath, chainId);
            // Generate recommendation
            const { recommendedAction, reasoning } = this.generateRecommendation(bestPath, profitAfterGas, confidenceScore);
            return {
    ...
    async simulateTriangularPath(tokens, routers, inputAmount, chainId) {
        try {
            // Simulate: TokenA -> TokenB
            const amountB = await this.simulateSwap(tokens[0], tokens[1], inputAmount, routers[0], chainId);
            if (amountB === 0n)
                return null;
            // Simulate: TokenB -> TokenC  
            const amountC = await this.simulateSwap(tokens[1], tokens[2], amountB, routers[1], chainId);
            if (amountC === 0n)
                return null;
            // Simulate: TokenC -> TokenA (completing the triangle)
            const finalAmount = await this.simulateSwap(tokens[2], tokens[0], amountC, routers[2], chainId);
            if (finalAmount === 0n)
                return null;
    ...
    async simulateSwap(tokenIn, tokenOut, amountIn, router, chainId) {
        try {
    ...
    estimateSlippage(amount, liquidityScore) {
    ...
    calculateRiskScore(tokens, routers, amount) {
        ...
        if (uniqueRouters === 1)
            riskScore += 20; // Single router = higher risk
        else if (uniqueRouters === 2)
            riskScore += 10;
        // Stable token paths are safer
        const stableTokens = ['USDC', 'USDT', 'DAI', 'FRAX'];
        const stableCount = tokens.filter(token => stableTokens.some(stable => token.toLowerCase().includes(stable.toLowerCase()))).length;
        riskScore -= (stableCount * 5); // Reduce risk for stable tokens
        return Math.max(5, Math.min(95, riskScore));
    }
    calculateConfidenceScore(path, chainId) {
        let confidence = 70; // Base confidence
        // Higher profitability increases confidence
        if (path.profitability > 100)
            confidence += 20; // >1% profit
        else if (path.profitability > 50)
            confidence += 10; // >0.5% profit
        ...
        const goodRouterCount = path.routers.filter(r => goodRouters.includes(r.name)).length;
        confidence += goodRouterCount * 5;
        return Math.max(30, Math.min(95, Math.floor(confidence)));
    }
    generateRecommendation(path, profitAfterGas, confidenceScore) {
        if (confidenceScore > 80 && profitAfterGas > (0, ethers_1.parseUnits)("0.02", 18)) {
            return {
                recommendedAction: 'execute',
                reasoning: `High confidence (${confidenceScore}%) with strong profit ${(0, ethers_1.formatUnits)(profitAfterGas, 18)} ETH`
            };
        }
        else if (confidenceScore > 60 && profitAfterGas > (0, ethers_1.parseUnits)("0.01", 18)) {
            return {
                recommendedAction: 'execute',
                reasoning: `Medium confidence (${confidenceScore}%) with acceptable profit ${(0, ethers_1.formatUnits)(profitAfterGas, 18)} ETH`
            };
        }
...
exports.TriangularArbManager = TriangularArbManager;
