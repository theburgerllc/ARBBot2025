...
exports.OraclePriceValidator = void 0;
...
class OraclePriceValidator {
    providers;
    config;
    priceFeeds; // token -> oracle addresses
    ...
    async validateTokenPrice(tokenA, tokenB, dexPrice, chainId, tradeSize) {
        const sources = [];
        const warnings = [];
        try {
            ...
            if (chainlinkPrice.status === 'fulfilled' && chainlinkPrice.value)
                sources.push(chainlinkPrice.value);
            if (uniswapTWAP.status === 'fulfilled' && uniswapTWAP.value)
                sources.push(uniswapTWAP.value);
            if (bandPrice.status === 'fulfilled' && bandPrice.value)
                sources.push(bandPrice.value);
            if (api3Price.status === 'fulfilled' && api3Price.value)
                sources.push(api3Price.value);
            if (coingeckoPrice.status === 'fulfilled' && coingeckoPrice.value)
                sources.push(coingeckoPrice.value);
            ...
            const manipulationIndicators = await this.detectManipulationPatterns(tokenA, tokenB, sources, tradeSize, chainId);
            const manipulationScore = this.calculateManipulationScore(manipulationIndicators, deviation);
            // Add manipulation warnings
            if (manipulationIndicators.volumeSpike)
                warnings.push('Unusual volume spike detected');
            if (manipulationIndicators.priceGap)
                warnings.push('Large price gap from recent history');
            if (manipulationIndicators.crossDEXDivergence)
                warnings.push('High cross-DEX price divergence');
            if (manipulationIndicators.liquidityDrain)
                warnings.push('Significant liquidity reduction detected');
    ...
    async detectManipulationPatterns(tokenA, tokenB, sources, tradeSize, chainId) {
        const indicators = {
            volumeSpike: false,
            priceGap: false,
    ...
    calculateManipulationScore(indicators, deviation) {
        let score = 0;
        ...
        if (indicators.volumeSpike)
            score += 20;
        if (indicators.priceGap)
            score += 25;
        if (indicators.crossDEXDivergence)
            score += 15;
        if (indicators.timeSeriesAnomaly)
            score += 20;
        if (indicators.liquidityDrain)
            score += 30;
        return Math.min(100, score);
    }
    generateRecommendation(riskLevel, manipulationScore, warningCount) {
    ...
    calculateConsensusPrice(sources) {
        if (sources.length === 0)
            return 0n;
        if (sources.length === 1)
    ...
    async getChainlinkPrice(tokenA, tokenB, chainId) {
        try {
            ...
            const priceFeed = new ethers_1.Contract(feedAddress, abi, provider);
    ...
    async getUniswapTWAP(tokenA, tokenB, chainId) {
        try {
            const provider = this.providers.get(chainId);
            if (!provider)
                return null;
            ...
            const poolContract = new ethers_1.Contract(poolAddress, poolAbi, provider);
            // Get 30-minute TWAP
            ...
            return {
                ...
                price: (0, ethers_1.parseUnits)(price.toString(), 18),
                timestamp: Date.now(),
    ...
    async getBandProtocolPrice(tokenA, tokenB, chainId) {
        try {
            // Band Protocol is mainly on specific chains
            if (chainId !== 1 && chainId !== 56)
                return null; // Ethereum, BSC
    ...
    async getAPI3Price(tokenA, tokenB, chainId) {
        try {
            // API3 implementation
    ...
    async getCoingeckoPrice(tokenA, tokenB, chainId) {
        try {
            ...
            if (!tokenAId || !tokenBId)
                return null;
            const response = await axios_1.default.get(`https://api.coingecko.com/api/v3/simple/price?ids=${tokenAId}&vs_currencies=${tokenBId}`, { timeout: 3000 });
    ...
    async checkVolumeSpike(tokenA, tokenB, chainId) {
        try {
    ...
    async checkPriceGap(tokenA, tokenB, currentPrice, chainId) {
    ...
    async checkCrossDEXDivergence(tokenA, tokenB, chainId) {
        try {
    ...
    async checkTimeSeriesAnomalies(tokenA, tokenB, chainId) {
        const pairKey = `${tokenA}_${tokenB}_${chainId}`;
        const history = this.priceHistory.get(pairKey) || [];
    ...
    async checkLiquidityDrain(tokenA, tokenB, chainId) {
        try {
    ...
    getChainlinkFeedAddress(tokenA, tokenB, chainId) {
        // Return Chainlink feed address for token pair
        // This would use a comprehensive mapping
        return null; // Placeholder
    }
    async getUniswapV3PoolAddress(tokenA, tokenB, chainId) {
    ...
    getCoingeckoTokenId(tokenAddress) {
    ...
    storePriceData(tokenA, tokenB, price, chainId) {
        const pairKey = `${tokenA}_${tokenB}_${chainId}`;
        const history = this.priceHistory.get(pairKey) || [];
        history.push({
            price,
    ...
    getPriceHistory(tokenA, tokenB, chainId) {
        const pairKey = `${tokenA}_${tokenB}_${chainId}`;
...
exports.OraclePriceValidator = OraclePriceValidator;
