...
export class ProductionMonitor {
  ...
  private async sendEmailAlert(alert: any): Promise<void> {
    try {
      ...
      await axios.post(process.env.EMAIL_ALERT_ENDPOINT, {
  ...
  async updateTradeMetrics(trade: {
    ...
    const profitETH = Number(ethers.formatEther(trade.profit));
    ...
    if (recentTrades.length > 0) {
      ...
      const totalProfit = recentTrades.reduce((sum, t) => sum + Number(ethers.formatEther(t.profit)), 0);
      const totalGas = recentTrades.reduce((sum, t) => sum + Number(ethers.formatEther(t.gasCost)), 0);
  ...
  private getRecentTrades(hours: number): TradeRecord[] {
  ...
  private updateDrawdown(): void {
    ...
    for (const trade of this.tradeHistory) {
      currentValue += Number(ethers.formatEther(trade.profit));
  ...
  private getSystemHealthScore(): number {
    ...
    if (this.metrics.successRate24h < 0.2) score -= 30;
    if (this.metrics.gasEfficiencyRatio < 2) score -= 20;
    if (this.metrics.currentDrawdown > 0.03) score -= 25;
    if (this.metrics.consecutiveFailures > 10) score -= 15;
    if (this.metrics.memoryUsageMB > 1500) score -= 10;
  ...
  public getRecentTradeHistory(hours: number = 24): TradeRecord[] {
    return this.getRecentTrades(hours);
  }

  public async generateDailyPerformanceReport(): Promise<any> {
