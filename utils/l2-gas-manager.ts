import { JsonRpcProvider, formatUnits, parseUnits, Interface } from "ethers";
...
export class L2GasManager {
    ...
    private readonly ARB_SYS_ADDRESS = "0x0000000000000000000000000000000000000064";
    private readonly ARB_GAS_INFO_ADDRESS = "0x000000000000000000000000000000000000006C";
    
    // Optimism system contracts
    private readonly OPT_GAS_ORACLE_ADDRESS = "0x420000000000000000000000000000000000000F";
    ...
    async getArbitrumGasCost(
        ...
        try {
            ...
            const fallbackCost = gasLimit * parseUnits("0.1", "gwei");
            return {
    ...
    async getOptimismGasCost(
        ...
        try {
            ...
            // Fallback calculation
            const fallbackCost = gasLimit * parseUnits("0.001", "gwei"); // Optimism is cheaper
            return {
    ...
    async estimateOptimalGasLimit(
        chainId: number,
        transactionType: 'simple-swap' | 'flash-arbitrage' | 'batch-arbitrage' | 'triangular-arbitrage'
    ): Promise<bigint> {
        const baseGasLimits = {
            'simple-swap': 300000n,
    ...
    async optimizeGasPriceForL2(
        ...
        if (chainId === 42161) { // Arbitrum
            ...
            let multiplier: number;
            switch (urgency) {
                case 'low': multiplier = 1.05; break;
                case 'medium': multiplier = 1.15; break;
                case 'high': multiplier = 1.25; break;
            }
            
            const maxFeePerGas = gasInfo.l2GasPrice * BigInt(Math.floor(multiplier * 100)) / 100n;
            const maxPriorityFeePerGas = parseUnits("0.001", "gwei"); // Minimal tip for Optimism
    ...
    private async getArbitrumGasInfo(): Promise<ArbitrumGasMetrics> {
        try {
            ...
            const gasInfoInterface = new Interface(gasInfoAbi);
            ...
            return {
                arbGasPrice: parseUnits("0.1", "gwei"), // Typical Arbitrum gas price
                l1BaseFee: l1BaseFee.gasPrice || parseUnits("20", "gwei"),
                l1GasPerByte: 16n, // Standard L1 gas per byte
                l2BaseFee: parseUnits("0.1", "gwei"),
                compressionRatio: 3 // Arbitrum compresses data ~3x
            };
        } catch (error) {
            // Fallback values
            return {
                arbGasPrice: parseUnits("0.1", "gwei"),
                l1BaseFee: parseUnits("20", "gwei"),
                l1GasPerByte: 16n,
                l2BaseFee: parseUnits("0.1", "gwei"),
                compressionRatio: 3
            };
    ...
    private async getOptimismGasInfo(): Promise<OptimismGasMetrics> {
        try {
            ...
            return {
                l1BaseFee: l1BaseFee.gasPrice || parseUnits("20", "gwei"),
                l1GasPrice: l1BaseFee.gasPrice || parseUnits("20", "gwei"),
                l2GasPrice: l2GasPrice.gasPrice || parseUnits("0.001", "gwei"),
            ...
            return {
                l1BaseFee: parseUnits("20", "gwei"),
                l1GasPrice: parseUnits("20", "gwei"),
                l2GasPrice: parseUnits("0.001", "gwei"),
                overhead: 2100n,
                scalar: 0.684
            };
        }
    ...
    private compressTransactionData(txData: string): string {
