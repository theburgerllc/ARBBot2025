...
export interface ProfitThresholdConfig {
    baseThresholdBps: number;   // Base profit threshold in basis points
    ...
    gasRatioMax: number;        // Max gas cost as % of profit
}
...
export class AdaptiveProfitManager {
    private config: ProfitThresholdConfig;
    private recentProfits: Array<{ 
        profit: bigint; 
        timestamp: number; 
        conditions: MarketConditions;
        success: boolean;
    ...
    async calculateOptimalThreshold(
        ...
        let recommendation: 'aggressive' | 'balanced' | 'conservative';
        if (finalThreshold < 25) recommendation = 'aggressive';
        else if (finalThreshold > 50) recommendation = 'conservative';
    ...
    private async assessVolatility(tokenPair: [string, string], chainId: number): Promise<'low' | 'medium' | 'high'> {
        try {
            ...
            if (avgVolatility < 0.02) return 'low';     // <2% daily volatility
            if (avgVolatility < 0.05) return 'medium';  // 2-5% daily volatility
    ...
    private async assessLiquidity(tokenPair: [string, string], chainId: number): Promise<'thin' | 'normal' | 'deep'> {
        try {
            // Estimate combined liquidity across major DEXs
            const totalLiquidity = await this.estimateTotalLiquidity(tokenPair, chainId);
            const liquidityETH = Number(formatUnits(totalLiquidity, 18));
            
            if (liquidityETH < 50) return 'thin';      // <50 ETH total liquidity
            if (liquidityETH < 500) return 'normal';   // 50-500 ETH
            return 'deep';                             // >500 ETH
    ...
    private async assessCompetition(chainId: number): Promise<'low' | 'medium' | 'high'> {
        try {
            // Estimate competition based on mempool activity and gas prices
            const provider = this.providers.get(chainId);
            if (!provider) return 'medium';
            
            const feeData = await provider.getFeeData();
            const gasPrice = Number(formatUnits(feeData.gasPrice || 0n, "gwei"));
            
            // Higher gas prices generally indicate more competition
            if (gasPrice < 20) return 'low';
            if (gasPrice < 80) return 'medium';
            return 'high';
    ...
    private async assessNetworkCongestion(chainId: number): Promise<'low' | 'medium' | 'high'> {
        try {
            const provider = this.providers.get(chainId);
            if (!provider) return 'medium';
            
            const feeData = await provider.getFeeData();
            const baseFee = Number(formatUnits(feeData.gasPrice || 0n, "gwei"));
            ...
            if (baseFee < thresholds[0]) return 'low';
            if (baseFee < thresholds[1]) return 'medium';
            return 'high';
    ...
    private assessTimeOfDay(): 'quiet' | 'active' | 'peak' {
        const hour = new Date().getUTCHours();
        
        // Define activity periods (UTC)
        if (hour >= 2 && hour < 8) return 'quiet';     // Asian quiet period
        if (hour >= 13 && hour < 17) return 'peak';    // US market hours
        if (hour >= 8 && hour < 13) return 'active';   // European hours
        return 'active';                                // Default to active
    }
    
    // OPTIMIZATION: Machine learning from recent performance
    private async applyMachineLearningAdjustment(conditions: MarketConditions): Promise<number> {
        ...
        if (recentData.length < 5) return 1.0; // Need more data
    ...
    private getRecentPerformanceForConditions(conditions: MarketConditions): Array<{
        profit: bigint;
        success: boolean;
        gasRatio: number;
        timestamp: number;
    ...
    private conditionsMatch(a: MarketConditions, b: MarketConditions): boolean {
    ...
    private recordOpportunity(profit: bigint, conditions: MarketConditions, gasRatio: number): void {
    ...
    updateTradeOutcome(
        ...
        const opportunity = this.recentProfits.find(p => 
    ...
    async getThresholdRecommendations(
        ...
        // Create variations
        const conservative: ProfitThresholdResult = {
            ...baseResult,
        ...
        const aggressive: ProfitThresholdResult = {
            ...baseResult,
    ...
    private async getTokenVolatility(token: string, chainId: number): Promise<number> {
        // Placeholder - would calculate from historical price data
        return 0.03; // 3% default volatility
    }
    
    private async estimateTotalLiquidity(tokenPair: [string, string], chainId: number): Promise<bigint> {
        // Placeholder - would query multiple DEX pools
        return parseUnits("100", 18); // 100 ETH equivalent
    }
    ...
    private calculateOpportunityCapture(): number {
