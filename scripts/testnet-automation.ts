...
interface TestnetConfig {
  ...
  bridgeUrl?: string;
  requiredBalance: string; // in ETH
  priority: number; // 1 = highest priority
  estimatedWaitTime: number; // minutes
  amount: string; // ETH per request
}
...
class TestnetAutomation {
  ...
  private readonly TESTNET_CONFIGS: TestnetConfig[] = [
    {
  ...
  async initialize(): Promise<void> {
    console.log(chalk.blue('🚀 Initializing Testnet Automation System'));
    ...
    console.log(chalk.green('✅ Browser automation initialized'));
  }

  async checkAllBalances(): Promise<{ [network: string]: string }> {
    console.log(chalk.blue('\n💰 Checking balances across all testnets...'));
    
    const balances: { [network: string]: string } = {};
    ...
    for (const config of this.TESTNET_CONFIGS) {
      try {
        ...
        console.log(chalk.white(
          `  ${status} ${config.name}: ${balanceEth} ETH (need ${config.requiredBalance} ETH)`
        ));
        
      } catch (error) {
        console.log(chalk.red(`  ❌ ${config.name}: Connection failed - ${error}`));
  ...
  async requestFromMultipleFaucets(config: TestnetConfig): Promise<FaucetResult> {
    ...
    console.log(chalk.white(`  📋 Found ${allFaucets.length} faucet sources to try`));
    
    // Try faucets in parallel (for speed) but with fallback logic
    const results: Promise<FaucetResult>[] = [];
    
    for (let i = 0; i < Math.min(3, allFaucets.length); i++) {
      const faucetUrl = allFaucets[i];
      const faucetConfig = { ...config, faucetUrl };
      
      console.log(chalk.gray(`  🚀 Attempting faucet ${i + 1}: ${faucetUrl.substring(0, 40)}...`));
    ...
    try {
      ...
      if (raceResult.success) {
        ...
        console.log(chalk.green(`  ✅ Success from ${faucetName} (source ${raceResult.sourceIndex + 1})`));
        return raceResult;
      }
    } catch (firstError) {
      console.log(chalk.yellow(`  ⚠️ First attempt failed, waiting for other attempts...`));
    }
    ...
    try {
      ...
      for (let i = 0; i < allResults.length; i++) {
        ...
        if (result.status === 'fulfilled' && result.value.success) {
          ...
          console.log(chalk.green(`  ✅ Success from ${faucetName} (fallback result)`));
          return result.value;
        }
      }
      ...
      const errors = allResults
        ...
        .map((r, i) => `${new URL(allFaucets[i]).hostname}: ${
          r.status === 'rejected' 
            ? r.reason 
            : (r.value as FaucetResult).error
        }`);
  ...
  async requestFaucetTokens(config: TestnetConfig, retryCount: number = 0): Promise<FaucetResult> {
    ...
    try {
      ...
      console.log(chalk.gray(`  📡 Navigating to ${config.faucetUrl}...`));
      ...
      if (result.success) {
        console.log(chalk.green(`  🎉 ${faucetName} request successful!`));
        return result;
      }
      
      // If not successful and we have retries left, retry
      if (retryCount < maxRetries) {
        console.log(chalk.yellow(`  ⚠️ ${faucetName} failed, retrying in 5 seconds...`));
      ...
      console.log(chalk.red(`  ❌ ${faucetName} error: ${errorMsg}`));
      ...
      if (retryCount < maxRetries && (
        ...
        console.log(chalk.yellow(`  🔄 Network error detected, retrying ${faucetName} in 10 seconds...`));
  ...
  private async handleChainlinkFaucet(config: TestnetConfig): Promise<FaucetResult> {
    ...
    try {
      ...
      try {
        ...
        console.log(chalk.yellow('  ⚠️ MetaMask not detected, trying manual address input'));
      }
      ...
      if (await requestButton.isVisible()) {
        ...
        console.log(chalk.green('  ✅ Faucet request submitted'));
  ...
  private async handlePK910Faucet(config: TestnetConfig): Promise<FaucetResult> {
    if (!this.page) throw new Error('Page not initialized');
    ...
    try {
      ...
      if (await startButton.isVisible()) {
        ...
        console.log(chalk.green('  ✅ Mining started (this may take several minutes)'));
  ...
  private async handleQuickNodeFaucet(config: TestnetConfig): Promise<FaucetResult> {
    if (!this.page) throw new Error('Page not initialized');
    ...
    try {
      ...
      console.log(chalk.green('  ✅ QuickNode faucet request submitted'));
  ...
  private async handleGenericFaucet(config: TestnetConfig): Promise<FaucetResult> {
    if (!this.page) throw new Error('Page not initialized');
    ...
    try {
      ...
      for (const selector of buttonSelectors) {
        try {
          ...
          console.log(chalk.green(`  ✅ Generic faucet request submitted for ${config.name}`));
  ...
  private async handleAlchemyFaucet(config: TestnetConfig): Promise<FaucetResult> {
    if (!this.page) throw new Error('Page not initialized');

    try {
      console.log(chalk.blue('  🔗 Using Alchemy faucet...'));
      ...
      for (const selector of requestSelectors) {
        try {
          await this.page.waitForSelector(selector, { timeout: 3000 });
          await this.page.click(selector);
          console.log(chalk.green('  ✅ Alchemy faucet request submitted'));
  ...
  private async handleBitbondFaucet(config: TestnetConfig): Promise<FaucetResult> {
    if (!this.page) throw new Error('Page not initialized');

    try {
      console.log(chalk.blue('  🔗 Using Bitbond faucet...'));
      ...
      for (const selector of claimSelectors) {
        try {
          await this.page.waitForSelector(selector, { timeout: 5000 });
          await this.page.click(selector);
          console.log(chalk.green('  ✅ Bitbond faucet request submitted'));
  ...
  private async handleLearnWeb3Faucet(config: TestnetConfig): Promise<FaucetResult> {
    if (!this.page) throw new Error('Page not initialized');

    try {
      console.log(chalk.blue('  🔗 Using LearnWeb3 faucet...'));
      ...
      for (const selector of requestSelectors) {
        try {
          await this.page.waitForSelector(selector, { timeout: 5000 });
          await this.page.click(selector);
          console.log(chalk.green('  ✅ LearnWeb3 faucet request submitted'));
  ...
  async automateEthereumToBridging(): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.yellow('  ⚠️ Bridge automation requires manual wallet connection'));
      console.log(chalk.cyan('  📋 Bridge URL opened: ' + bridgeUrl));
      console.log(chalk.cyan('  💡 Manually connect wallet and bridge funds if needed'));

    } catch (error) {
      console.log(chalk.red(`❌ Bridge navigation failed: ${error}`));
    }
  }

  async updateEnvironmentFile(balances: { [network: string]: string }): Promise<void> {
    ...
    console.log(chalk.green('  ✅ Environment file updated with testnet configuration'));
  }

  async generateReport(balances: { [network: string]: string }, faucetResults: FaucetResult[]): Promise<void> {
    const report = {
    ...
    for (const [network, balance] of Object.entries(balances)) {
      ...
      const status = hasEnough ? '✅' : '❌';
      console.log(chalk.white(`  ${status} ${network}: ${balance} ETH`));
    }

    console.log(chalk.cyan('\n🚰 Faucet Results:'));
    faucetResults.forEach((result, i) => {
      const status = result.success ? '✅' : '❌';
      console.log(chalk.white(`  ${status} Request ${i + 1}: ${result.success ? 'Success' : result.error}`));
    });
  ...
  private generateRecommendations(balances: { [network: string]: string }): string[] {
  ...
  async cleanup(): Promise<void> {
    if (this.browser) {
      ...
      console.log(chalk.green('🔄 Browser automation cleaned up'));
    }
  }
  ...
  async runFullAutomation(): Promise<void> {
    try {
      ...
      const faucetResults: FaucetResult[] = [];
      
      if (needsFunding.length > 0) {
        console.log(chalk.yellow(`\n🚰 ${needsFunding.length} networks need funding (priority order):`));
        needsFunding.forEach((config, i) => {
          console.log(chalk.white(`  ${i + 1}. ${config.name} (Priority: ${config.priority}, Need: ${config.requiredBalance} ETH)`));
        });
        
        // Focus on Arbitrum Sepolia first (highest priority for deployment)
        for (const config of needsFunding) {
          console.log(chalk.magenta(`\n${'='.repeat(60)}`));
          console.log(chalk.magenta(`🎯 Targeting ${config.name.toUpperCase()} (Priority ${config.priority})`));
          ...
          if (config.name === 'Arbitrum Sepolia' && result.success) {
            ...
            console.log(chalk.cyan(`⏭️ Continuing with other networks for completeness...`));
          }
      ...
      console.log(chalk.red(`❌ Automation failed: ${error}`));
      throw error;
    } finally {
      await this.cleanup();
    }
