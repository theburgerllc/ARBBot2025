...
import { DynamicThresholdOptimizer } from '../utils/dynamic-threshold-optimizer';
...
async function testOptimizedThresholds() {
  ...
  try {
    ...
    const optimizer = new DynamicThresholdOptimizer(provider);
...
═══════════════════════════`);
    ...
    for (const scenario of gasPriceScenarios) {
      const mockConditions = {
        ...conditions,
        gasPrice: ethers.parseUnits(scenario.price, 'gwei')
      };
